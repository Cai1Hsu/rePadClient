...
/* loaded from: classes.jar:android/support/v4/widget/AutoScrollHelper.class */
public abstract class AutoScrollHelper implements View.OnTouchListener {
    ...
    /* loaded from: classes.jar:android/support/v4/widget/AutoScrollHelper$ClampedScroller.class */
        ...
        private float getValueAt(long j) {
            ...
            if (j >= this.mStartTime) {
                if (this.mStopTime < 0 || j < this.mStopTime) {
                    f = AutoScrollHelper.constrain(((float) (j - this.mStartTime)) / this.mRampUpDuration, 0.0f, (float) AutoScrollHelper.DEFAULT_RELATIVE_VELOCITY) * 0.5f;
                } else {
                    f = (AutoScrollHelper.constrain(((float) (j - this.mStopTime)) / this.mEffectiveRampDown, 0.0f, (float) AutoScrollHelper.DEFAULT_RELATIVE_VELOCITY) * this.mStopValue) + (AutoScrollHelper.DEFAULT_RELATIVE_VELOCITY - this.mStopValue);
                }
            }
            return f;
        }
    ...
    /* loaded from: classes.jar:android/support/v4/widget/AutoScrollHelper$ScrollAnimationRunnable.class */
    private class ScrollAnimationRunnable implements Runnable {
        ScrollAnimationRunnable() {
            AutoScrollHelper.this = r4;
        }
    ...
    private float computeTargetVelocity(int i, float f, float f2, float f3) {
        ...
        if (edgeValue == 0.0f) {
            ...
            float f7 = f4 * f3;
            constrain = edgeValue > 0.0f ? constrain(edgeValue * f7, f5, f6) : -constrain((-edgeValue) * f7, f5, f6);
        }
        return constrain;
    }
                ...
                case 1:
                    ...
                    if (f < f2) {
                        if (f < 0.0f) {
                            ...
                            f3 = DEFAULT_RELATIVE_VELOCITY - (f / f2);
                            break;
                        }
                    }
                    break;
    ...
    private float getEdgeValue(float f, float f2, float f3, float f4) {
        ...
        interpolation = -this.mEdgeInterpolator.getInterpolation(-constrainEdgeValue);
        f5 = constrain(interpolation, -1.0f, (float) DEFAULT_RELATIVE_VELOCITY);
        return f5;
    }
