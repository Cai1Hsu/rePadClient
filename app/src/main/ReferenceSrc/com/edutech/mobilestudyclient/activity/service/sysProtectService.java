...
public class sysProtectService extends Service {
    ...
    private final BroadcastReceiver APKInstallReceiver = new BroadcastReceiver() { // from class: com.edutech.mobilestudyclient.activity.service.sysProtectService.2
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals("android.intent.action.PACKAGE_ADDED")) {
                ...
                boolean hasApp = sysProtectService.this.addedApps != null && sysProtectService.this.addedApps.contains(packageName2);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals("PASSWDPASS")) {
                ...
                if (action.equals("ENTERPASS")) {
                    ...
                    if (packageName != null && !packageName.equals("") && !sysProtectService.this.addedApps.contains(packageName)) {
                        sysProtectService.this.addedApps.add(packageName);
                    }
                    sysProtectService.this.installNewApk(path);
                    ...
                    if (pkgs != null && pkgs.size() > 0) {
                        for (int i = 0; i < pkgs.size(); i++) {
                            try {
                                if (!sysProtectService.this.addedApps.contains(pkgs.get(i))) {
                                    sysProtectService.this.addedApps.add(pkgs.get(i));
                                }
                            } catch (Exception e) {
                                return;
                            }
    ...
    public void showDialog(String state) {
        ...
        if (canshow && this.blackDialog == null) {
            ...
            wifiTxt.setOnClickListener(new View.OnClickListener() { // from class: com.edutech.mobilestudyclient.activity.service.sysProtectService.4
